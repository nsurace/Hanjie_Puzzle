...
function init() {
   ...
   //Loop through the array of elements with the class puzzles
...
function swapPuzzle(e){
   if (window.confirm("You will lose all of your work on the puzzle! Continue??")){
...
function setupPuzzle(){
   ...
   for (var i = 0; i < puzzleCells.length; i++) {
      ...
      //set the cell background color in response to the mouse event
   ...
   document.getElementById("peek").addEventListener("click",
      function(){
         ...
         //Remove the hint after 0.5 seconds
         setTimeout(
            function() {
               //Change pink cells back to white and red cells back to gray
               for(var i = 0; i < puzzleCells.length; i++){
...
function setBackground(e) {
   ...
   if (e.shiftKey){
      cellBackground = "rgb(233, 207, 29)"; //gold
      cursorType = "url(jpf_eraser.png), cell";
   } else if (e.altKey) {
      cellBackground = "rgb(255, 255, 255)"; //white
      cursorType = "url(jpf_cross.png), crosshair";
   } else {
      cellBackground = "rgb(101, 101, 101)";  //gray
   ...
   //Prevent the default action of the selecting table heading text
   e.preventDefault();
}
